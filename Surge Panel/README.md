...
- 点按组件右上角刷新按钮会通过 AppIntent 标记强制重取并请求 WidgetKit 重载；点按组件其它区域打开 Surge Panel、或切回 Scripting 首页 Tab 时也会触发同样的强制刷新（系统仍可能短暂延后）
...
- 大 / 中组件的双列卡片按 `Widget.displaySize` 计算等宽并使用固定行高；小组件压缩成出口摘要与四项健康指标，避免不同设备上出现卡片高低不一或大面积留白
